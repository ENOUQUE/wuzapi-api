...
func TestChatSendButtons(t *testing.T) {
	s := makeTestServer(t)
	...
	buttonsRequest := newRequest("2", "chat.send.buttons", map[string]interface{}{
		"token": "buttons-token",
		"Phone": "1234567890",
	...
	response := executeRequest(t, s, buttonsRequest)
	...
	// Should error due to no WhatsApp session
	if response["error"] == nil {
		t.Errorf("Expected error (no session), got success")
	}
}

func TestChatSendList(t *testing.T) {
	s := makeTestServer(t)
	...
	// Send list message
	listRequest := newRequest("2", "chat.send.list", map[string]interface{}{
		"token":       "list-token",
		"Phone":       "1234567890",
	...
	response := executeRequest(t, s, listRequest)
